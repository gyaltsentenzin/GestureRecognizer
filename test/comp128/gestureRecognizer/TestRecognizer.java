...
public class TestRecognizer {
   ...
   //Test centroid and boundingBox methods
   /**
    *  check if the centroid calculation is accurate 
    */
    @Test
    public void testCentroid(){
        Point centroid = recognizer.centroidCalc(originalPoints);
        assertEquals(new Point(((double)ORIGINAL_N-1)/2, 0), centroid);
    }

    /**
     * Test to address if the bounds are recorded accurately
     */
    @Test
    public void testBoundingBox(){
        Point boundingBox = recognizer.getBoundingSize(originalPoints);
        assertEquals(new Point(ORIGINAL_N-1,0), boundingBox);
    }
   ...
   public void testRecognize(){
       ...
       Match selfMatch = recognizer.recognize(circleTemplate);

       score = selfMatch.getMatchScore();

       //set score to the new recognition score
       assertEquals(1.0, score, 0.01); // A template matched with itself should be a perfect match 

       Match selfRecognition2 = recognizer.recognize(circleTemplate);

       score = selfRecognition2.getMatchScore();
       assertEquals(1.0, score, 0.01); // A template matched with itself should be a perfect match 
